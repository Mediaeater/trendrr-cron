...
public class SchedulingPattern {
	...
	/**
	 * Builds a SchedulingPattern parsing it from a string. Defaults to EST timezone (I know, I know, but we like it that way :))
	 * 
	 * @param pattern
	...
	public Date getNextMatchingDate(Date start){
		...
		//stop is one year + 1 minute.
		Date stop = Timeframe.MINUTES.add(Timeframe.YEARS.add(start, 1), 1);
		while(maybeMatch.before(stop)){
	 ...
	 * @return
	 */
	public Date getPrevMatchingDate(Date start){
		Date maybeMatch = start;
		//stop is one year + 1 minute.
		Date stop = Timeframe.MINUTES.add(Timeframe.YEARS.add(start, -1), -1);
		while(maybeMatch.after(stop)){
			maybeMatch = Timeframe.MINUTES.add(maybeMatch, -1);
			if(this.match(maybeMatch))
				return maybeMatch;
